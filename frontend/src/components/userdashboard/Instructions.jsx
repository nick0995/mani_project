...
function Instructions({ onStartTest, username }) {
  const [lang, setLang] = useState('en');
  const t = translations[lang];
  const navigate = useNavigate();

  const handleStart = () => {
    if (onStartTest) onStartTest(); // trigger testStart state in App.js
    navigate("/TestPage"); // ✅ go to TestPage without refresh
  };

  return (
    <div id="instructionPage" className="instruction-container">
      {/* Language Switch + Username */}
      ...
      <div className="flex flex-col items-center mt-8">
        ...
        <button onClick={handleStart} className="btn btn-outline-primary mt-4">
          {t.start}
        </button>
      </div>
    </div>
